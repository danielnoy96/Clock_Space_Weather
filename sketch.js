...
const COL = {
  ...
  xray:      [180, 120, 255], // violet
  mag:       [120, 220, 255], // cyan
  h_ions:    [80, 120, 255],  // deep blue
  electrons: [140, 255, 170], // green
...
function spawnXrayPulse(T, spikeStrength) {
...
function seedChamberParticles(T, count) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    let size = 1.45;
    if (kind === "xray") size = 1.9;
    else if (kind === "electrons") size = 1.6;
    else if (kind === "mag") size = 1.5;
    else if (kind === "h_ions") size = 1.4;
    else if (kind === "protons") size = 1.45;
...
function applyCalmOrbit(p, center) {
  ...
  const swirl = 0.18 + 0.45 * mag + 0.25 * protons;         // calmer baseline; less ring-locking
  const jitter = 0.15 + 1.10 * electrons;                   // electrons add micro-turbulence
  const soften = 1.0 - 0.65 * protons;                       // high protons = less distortion

  p.vel.add(tang.mult(swirl * 0.40));
  p.vel.add(p5.Vector.random2D().mult(jitter * 0.05 * soften));
}
...
function updateAudioFeatures() {
  ...
  // xray spike -> burst
  const spike = xray - prevXray;
  if (spike > 0.14) triggerXrayBurst(null, constrain(spike * 4.5, 0, 1)); // slightly easier trigger
  prevXray = xray;
}
...
function fallbackFeatures() {
  ...
  if (random() < 0.006) triggerXrayBurst(null, 0.45);
}
...
function emitFromHand(T, which, rate) {
  ...
  const spread = (1.0 + electrons * 2.2 + mag * 1.2) * (1.0 - stiffness * 0.70);
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (kind === "xray")      { size = 1.9; }
    if (kind === "electrons") { size = 1.6; }
    if (kind === "mag")       { size = 1.5; }
    if (kind === "h_ions")    { size = 1.4; }
    if (kind === "protons")   { size = 1.45; }
...
function updateParticles(T) {
  ...
  for (let i = particles.length - 1; i >= 0; i--) {
    ...
    if (!disableFrameForces) {
      ...
      if ((i % HEAVY_FIELD_STRIDE) === heavyPhase) {
        applyEddyField(p, T);
...
Particle.prototype.draw = function() {
  ...
  const alpha = (prof.alphaBase + prof.alphaStrength * strength) * a * flick;
